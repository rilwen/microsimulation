...
#ifndef __AVERISERA_INTERP_POLYNOMIAL_INTERPOLATION_H
#define __AVERISERA_INTERP_POLYNOMIAL_INTERPOLATION_H
...
namespace averisera {
		...
		namespace PolynomialInterpolation
		{
			...
			template <unsigned int N> double evaluate_poly(double x, const std::array<double, N>& a) {
				double result = 0.0;
				unsigned int i = N;
				while (i > 0) {
					--i;
...
#endif // __AVERISERA_INTERP_POLYNOMIAL_INTERPOLATION_H

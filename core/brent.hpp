#ifndef __AVERISERA_BRENT_H
#define __AVERISERA_BRENT_H
...
namespace averisera {
    namespace RootFinding {
        ...
        template <typename F>
        inline double safe_evaluate(F fun, double x)
        {
        ...
        {
            if (xtol < 0 || ytol < 0) {
                throw std::runtime_error("StoppingConditions cannot be negative");
            }
        ...
        // Throws std::runtime_error if not converged or supplied bounds do not bracket a root.
        template <typename F>
        double find_root(F f, double xa, double xb, double xtol, double ytol)
        {

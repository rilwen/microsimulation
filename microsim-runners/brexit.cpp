...
/** Returns concenption hazard rate multipliers correcting fertility rates for ethnic groups.
...
@return An object providing conception hazard rate multipliers for different ethnic groups to adjust their fertility rates.
...
@throw DataException If loaded gender ratios are out of [0, 1] range.
*/
static std::vector<std::unique_ptr<Operator<Person>>> build_fetus_generator_operators(const std::string& birth_gender_ratios_file) {
	...
	for (size_t r = 0; r < gender_rates.nbr_rows(); ++r) {
		...
		if (p >= 0 && p <= 1) {
			...
			throw DataException("Gender ratio out of [0, 1] range.");
		}
	}
...
/** Generates names of files with census data (original or generated by our microsimulation).

@param census_years Vector of years for which we want censuses.
@param census_spacing_years Number of years between censuses.
@param resource_dir Path to resource directory containing data files.
@param pattern
@param multiple_simulations Whether we are loading census data that were generated by a set of simulations spanning census_spacing_years each.

@return Names of files.
*/
static std::vector<std::string> get_census_filenames(const std::vector<Date::year_type>& census_years, const unsigned int census_spacing_years, const std::string& resource_dir, const std::string& pattern, const bool multiple_simulations) {
	std::vector<std::string> census_filenames;
	for (auto year : census_years) {
		if (!multiple_simulations) {
			// Loading original census data, or generated by a single simulation.
			census_filenames.push_back(resource_dir + boost::str(boost::format(pattern) % year));
		} else {
			// Loading generated census data, from multiple simulations.
			const auto simulation_end_year = year + census_spacing_years;
			census_filenames.push_back(resource_dir + boost::str(boost::format(pattern) % year % simulation_end_year));
		}
	}
...
typedef std::vector<PopulationCalibrator::pop_data_type> pop_data_vector; /**< Vector of dataframes describing population broken down by ethnicity and age group. */

/** Calculates total population size.

@param female Dataframe describing the female population.
@param male Dataframe describing the male population.
...
static void get_single_simulation_no_migration_numbers(const std::vector<Date::year_type>& census_years_start,
	...
	const std::vector<std::string> nomigr_female_census_filenames(get_census_filenames(census_years_start, 0, resource_dir, "nomigr_census_%d_female.csv", false));
	const std::vector<std::string> nomigr_male_census_filenames(get_census_filenames(census_years_start, 0, resource_dir, "nomigr_census_%d_male.csv", false));
...
static void get_multiple_simulations_no_migration_numbers(const std::vector<Date::year_type>& census_years,
	...
	const std::vector<std::string> nomigr_female_census_filenames_start(get_census_filenames(census_years_start, census_spacing_years, resource_dir, "nomigr_census_%d_%d_female_start.csv", true));
	const std::vector<std::string> nomigr_male_census_filenames_start(get_census_filenames(census_years_start, census_spacing_years, resource_dir, "nomigr_census_%d_%d_male_start.csv", true));
	//const std::vector<Date::year_type> census_years_end(census_years.begin() + 1, census_years.end());
	const std::vector<std::string> nomigr_female_census_filenames_end(get_census_filenames(census_years_start, census_spacing_years, resource_dir, "nomigr_census_%d_%d_female_end.csv", true));
	const std::vector<std::string> nomigr_male_census_filenames_end(get_census_filenames(census_years_start, census_spacing_years, resource_dir, "nomigr_census_%d_%d_male_end.csv", true));
...
void do_main(const UserArguments& ua) {
	...
	const std::vector<std::string> female_census_filenames(get_census_filenames(census_years, census_year_spacing, resource_dir, "census_%d_female.csv", false));
	const std::vector<std::string> male_census_filenames(get_census_filenames(census_years, census_year_spacing, resource_dir, "census_%d_male.csv", false));

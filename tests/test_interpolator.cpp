...
#include "core/interpolator_impl_constant.hpp"
#include "core/interpolator_impl_piecewise_constant.hpp"
#include "core/interpolator_impl_factory.hpp"
...
TEST_F(InterpolatorTest, FritschButland)
{
	const unsigned int n = 10;
	std::vector<double> x(n);
	std::vector<double> y(n);
	...
	Interpolator interp(std::shared_ptr<InterpolatorImpl>(InterpolatorImplFactory::fritschButland(x, y)));
	for (unsigned int i = 0; i < n; ++i)
	{
		EXPECT_NEAR(y[i], interp(x[i]), 5E-16);
	}
	for (double v = x.front(); v <= x.back(); v += 0.01)
	{
		EXPECT_NEAR(fun(v), interp(v), 0.1);
...
TEST_F(InterpolatorTest, InterpolateYLinearly) {
	const std::vector<double> x({ 0., 1., 2. });
	const std::vector<double> y({ 0., 0.4, 1.0 });
	const std::vector<double> new_x({ 0., 0.25, 1., 1.5, 2. });
	const std::vector<double> new_y_act(Interpolator::interpolate_y_linearly(x, y, new_x));
	const std::vector<double> new_y_exp({ 0., 0.1, 0.4, 0.7, 1.0 });
	ASSERT_EQ(new_y_exp.size(), new_y_act.size());
	ASSERT_EQ(0., new_y_act.front());
	ASSERT_EQ(1., new_y_act.back());
	for (size_t i = 0; i < new_y_exp.size(); ++i) {
		ASSERT_NEAR(new_y_exp[i], new_y_act[i], 1E-15) << i;
	}	
}
